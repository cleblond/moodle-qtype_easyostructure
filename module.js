...
M.qtype_easyostructure={
    ...
    show_error : function (Y, topnode) {
        var errormessage = '<span class ="javawarning">'
            +M.util.get_string('enablejava', 'qtype_easyostructure')+
            '</span>';
 ...
 // Note: This method is also called from mod/audiorecorder
	...
	newApplet.setAttribute('lonePairsVisible','true');
	}
	else if (displaymode=='2'){
	newApplet.setAttribute('implicitH','all');
	newApplet.setAttribute('viewCarbonVisibility','on');
	newApplet.setAttribute('navmode','rot3d');
	newApplet.setAttribute('importConv','H+');
	newApplet.setAttribute('rendering','ballstick');
